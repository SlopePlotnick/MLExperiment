...
#
# model_base = keras.Model(inputs=inp, outputs=out)
...
# Fit the model
# history_base = model_base.fit(np.array(X_train),
#                          y_train,
#                          batch_size = 64,
#                          verbose = 1,
...
history_optimized = model_optimized.fit(np.array(X_train),
                                        y_train,
                                        batch_size=64,
                                        verbose=1,
...
# print("Base Model Test_Accuracy: {:.2f}%".format(model_base.evaluate(np.array(X_test), np.array(y_test))[1]*100))
print("Optimized Model Test_Accuracy: {:.2f}%".format(
    model_optimized.evaluate(np.array(X_test), np.array(y_test))[1] * 100))
...
# ax1.plot(epoch_list, history_base.history['accuracy'], label='Base-Train Accuracy')
# ax1.plot(epoch_list, history_base.history['val_accuracy'], label='Base-Validation Accuracy')
ax1.plot(epoch_list, history_optimized.history['accuracy'], label='Optimized-Train Accuracy')
ax1.plot(epoch_list, history_optimized.history['val_accuracy'], label='Optimized-Validation Accuracy')
...
# ax2.plot(epoch_list, history_base.history['loss'], label='Base-Train Loss')
# ax2.plot(epoch_list, history_base.history['val_loss'], label='Base-Validation Loss')
ax2.plot(epoch_list, history_optimized.history['loss'], label='Optimized-Train Loss')
ax2.plot(epoch_list, history_optimized.history['val_loss'], label='Optimized-Validation Loss')
...
# Save the model
# model_base.save('malaria_cnn_base.h5')
